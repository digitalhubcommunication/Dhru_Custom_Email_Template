...
<title>{$companyname} - New Order Placed</title>
</head>
...
<div style="max-width:650px; margin:30px auto; background:#ffffff; border-radius:10px; overflow:hidden; box-shadow:0 4px 15px rgba(0,0,0,0.08);">
    ...
    <div style="padding:30px; color:#333; font-size:15px; line-height:1.6;">
        ...
        <p style="margin:0 0 18px 0;">
            A new order of <strong>{$service}</strong> has been placed from your account.
        </p>

        {if $imeilist}
        <div style="background:#f9f9f9; border:1px solid #eee; border-radius:8px; padding:15px; margin:15px 0; color:#555;">
            <p style="margin:0;"><strong>IMEI List:</strong> {$imeilist}</p>
        </div>
        {/if}
